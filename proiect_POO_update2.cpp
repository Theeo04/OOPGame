...
void decision2(int tura, Character& Ch, villain& V1) {
    int alegere;
    cout << "Choose your attack:" << endl;
    cin >> alegere;
    ...
    if (tura % 2 == 0) { // randul lui Ch1
        if (alegere == 1) // close attack
            Ch.close_attack(Ch, V1);
        if (alegere == 2)
            Ch.far_attack(Ch, V1);
        if (alegere == 3)
            Ch.special_attack(Ch, V1);
        if (alegere == 4)
        	Ch.receive_health(Ch);
    }
    else { // randul lui V1
        if (alegere == 1) // close attack
            V1.close_attack(V1, Ch);
        if (alegere == 2)
            V1.far_attack(V1, Ch);
        if (alegere == 3)
            V1.special_attack(V1, Ch);
        if (alegere == 4)
...
int main()
{
	...
	if(gamemode == 1)
	{
		...
		villain v3("Nirnasha", 130 , 120, 80 , "Consecration" , "Ashes", " Conqueror of the Storm");
